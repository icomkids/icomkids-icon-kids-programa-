...
async function sendInvite(email: string, fullName: string, actionLink: string, chapterName: string) {
  ...
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ from, to: [email], subject: `Convite para liderar o ${chapterName} — ADHONEP`, html }) });
